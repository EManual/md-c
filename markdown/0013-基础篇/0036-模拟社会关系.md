...
struct person{
	...
	char sex;[Tags]/*性别：男用字符'M'；女用字符'F'*/
...
{
	int index;
	for(index=0;index<CHILDREN-1;index++)[Tags]/*寻找一个空缺的子女指针*/
		if(father->children[index]==NULL)[Tags]/*若没有空缺，则填在最后*/
			break;
...
mother_child(struct person *mother,struct person *child)
{
	int index;
	for(index=0;index<CHILDREN-1;index++)[Tags]/*寻找一个空缺的子女指针*/
		if(mother->children[index]==NULL)[Tags]/*若没有空缺，则填在最后*/
			break;
...
int brothersinlaw(struct person *p1,struct person *p2)
{
	...
	if(p1->sex==p2->sex) return 0;[Tags]/*不可能是堂兄妹*/
	f1=p1->father;
	f2=p2->father;
	if(f1!=NULL&&f1==f2) return 0;[Tags]/*是兄妹，不是堂兄妹*/
	while(f1!=NULL&&f2!=NULL&&f1!=f2)[Tags]/*考虑远房情况*/
	{
		f1=f1->father;
		f2=f2->father;
...
void print_relate(struct person *p)
{
	...
	if(p->children!=NULL)
	{	for(index=0;index<CHILDREN-1;index++)[Tags]/*寻找一个空缺的子女指针*/
		if(p->children[index]==NULL)[Tags]/*若没有空缺，index为子女个数 */
			break;
		if(index>0)
			printf(" Children are:");
		for(i=0;i<index;i++)
