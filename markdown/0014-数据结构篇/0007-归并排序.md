...
{[Tags]/* 将两个有序的子文件R[low..m)和R[m+1..high]归并成一个有序的 */
     /* 子文件R[low..high] */
     int i=low,j=m+1,p=0; [Tags]/* 置初始值 */
     int *R1; [Tags]/* R1是局部向量，若p定义为此类型指针速度更快 */
     ...
     while(i<=m&&j<=high) [Tags]/* 两子文件非空时取其小者输出到R1[p]上 */
       R1[p++]=(R[i]<=R[j])?R[i++]:R[j++];
     while(i<=m) [Tags]/* 若第1个子文件非空，则复制剩余记录到R1中 */
       R1[p++]=R[i++];
     while(j<=high) [Tags]/* 若第2个子文件非空，则复制剩余记录到R1中 */
       R1[p++]=R[j++];
