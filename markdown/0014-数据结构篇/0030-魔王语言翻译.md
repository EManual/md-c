...
{ 
	...
	for(i=0;c!='';i++)/*遍历数组*/
	{
		...
		if(c=='(')/*遇到开括号*/
		{
			...
			}while(c!=')');[Tags]/*直到括号中元素全部进栈*/ 
			pop(temps,&t);[Tags]/*将多余进栈的首字母t出栈*/
			pop(temps,&t);   [Tags]/*将多余进栈的')'出栈*/
		} 
	} 
}[Tags]/*临时栈*/ 
[Tags]/*特殊入队函数*/
...
{     
	...
	for(i=0;demon[   i]!='';i++)/*遍历数组*/ 
	{
		...
		[Tags]/*如果是小写字母或者'?'　则直接进栈*/     
		{
			enqueue(q,b.data);
		} 
		else 
		{
			if('A'<=b.data&&b.data<='Z')   [Tags]/*如果是大写字母,则调用特殊进栈函数,*/     
			{
				spenqueue(q,b.data);
			...
			else 
			{
				if(b.data=='(')/*如果是括号*/ 
				{
					do 
					{
						pop(s,&e);
						enqueue(q,e);
					}while(!(s->top==s->base));   [Tags]/*只要栈不为空,则出栈进队*/     
					while   (b.data!=')')   
...
{ 
	...
	while   (flag==1)   [Tags]/*如果有大写字母*/ 
	{
		k=0;
		flag=sort(&s1,&q1);
		while(q1.front!=q1.rear)   [Tags]/*重写demon[i   ]*/ 
		{
			dequeue(&q1,&e);
			demon[k]=e;
			k++;
