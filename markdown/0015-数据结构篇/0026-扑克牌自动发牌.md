...
void main()
{
    ...
    while(t<=52)      [Tags]/*控制发52张牌*/
    {     m=random(52);      [Tags]/*产生0到51之间的随机数*/
          for(flag=1,i=1;i<=t&&flag;i++)   [Tags]/*查找新产生的随机数是否已经存在*/
            if(m==a[i]) flag=0;     [Tags]/*flag=1:产生的是新的随机数
                                        [Tags]flag=0:新产生的随机数已经存在*/
          if(flag)
          {
             a[t++]=m;       [Tags]/*如果产生了新的随机数，则存入数组*/
             if(t%4==0) b1[b11++]=a[t-1];        [Tags]/*根据t的模值，判断当前*/
             else if(t%4==1) b2[b22++]=a[t-1];   [Tags]/*的牌应存入哪个数组中*/
             else if(t%4==2) b3[b33++]=a[t-1];
             else if(t%4==3) b4[b44++]=a[t-1];
          }
      }
      qsort(b1,13,sizeof(int),comp);      [Tags]/*将每个人的牌进行排序*/
...
void p(int p,int b[],char n[])
{
   ...
   for(i=0;i<13;i++)      [Tags]/*将数组中的值转换为相应的花色*/
      if(b[i]/13==0) printf(" %c",n[b[i]%13]);  [Tags]/*该花色对应的牌*/
   printf("\n                 \003 ");      [Tags]/*打印红桃标记*/
   for(i=0;i<13;i++)
...
int comp(const void *j,const void *i)  [Tags]/*qsort调用的排序函数*/
{
   return(*(int*)i-*(int*)j);
}
```