...
void main()
{
    ...
    for(i=1001;i<9999;i+=2)
    {
        ...
        if(k%2!=0&&k!=5&&num(i))
        {
            ...
            /*和最后一行的条件，记录可逆素数的*/
            /*下标，计数器加1*/
            if(number[count-1][2]%2!=0&&number[count-1][3]%2!=0&&number[count-1][2]!=5&&number[count-1][3]!=5)
        ...
        for(array[1][0]=0;array[1][0]<count;array[1][0]++)
        {
            ...
            for(array[2][0]=0;array[2][0]<count;array[2][0]++)
            {
                ...
                for(i4=0;i4<selecount;i4++)
                {
                    ...
                    [Tags]/*复制分解的数字*/
                    [Tags]/*判断每列是否可逆素数*/
                    for(flag=1,i=1;flag&&i<=4;i++)if(!find1(i))flag=0 ;
                    [Tags]/*判断对角线是否为可逆素数*/
                    if(flag&&find2())
                    {
                        printf("No.%d\n",++cc);
...
int num(int number)
{
    ...
    [Tags]/*将素数变为反序数*/
    for(j=0;number>0;number/=10)j=j*10+number%10 ;
    if(!ok(j))return 0 ;
    [Tags]/*判断反序数是否为素数*/
    return 1 ;
}
...
int comp_num(int n)
{
    ...
    {
        [Tags]/*计算前n位数字代表的数值*/
        for(num=0,k=0;k<n;k++)num=num*10+array[k][i];
        [Tags]/*与前一次最后查找到的元素进行比较*/
        if(num<=larray[n-2][*p])for(;*p>=0&&num<larray[n-2][*p];(*p)--);
