...
main()
{
	...
	while(r!=NULL)
	{
		...
		q=p;
		p=r;
		if(r->next!=NULL) [Tags]/*r后面还有结点，则逆置继续*/
			r=r->next;
		else
			break;
	}
	...
	while(q->next!=NULL) [Tags]/*若q所指向的元素后面还有其它元素，则将该元素的数据输出*/
	{
		printf("%c-->",q->next->c); [Tags]/*q->next->c表示q所指向的下一个元素的数据*/
